...
export default function InfoPanel({ word, onClose }: InfoPanelProps) {
  ...
  // Fetch definition whenever word changes
  useEffect(() => {
    if (!word) return;
    setDefinition(null);
    setMessages([]);
  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      const delta = startX.current - e.clientX; // dragging left → panel gets wider
      const newWidth = Math.max(300, Math.min(640, startWidth.current + delta));
  ...
  return (
    ...
    >
      {/* Drag handle */}
      <div
        className="w-1.5 cursor-ew-resize bg-verint-purple-pale hover:bg-verint-purple transition-colors flex-shrink-0"
        onMouseDown={onDragStart}
        title="Drag to resize panel"
      />

      {/* Panel body */}
      <div className="flex flex-col flex-1 bg-white border-l border-verint-purple-pale overflow-hidden">

        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 bg-verint-gradient flex-shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-white font-bold text-sm truncate capitalize">{word.word}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize flex-shrink-0 ${CATEGORY_BADGE[word.category] || CATEGORY_BADGE.general}`}>
              {word.category}
            </span>
        ...
        <div className="px-4 py-3 bg-verint-purple-bg border-b border-verint-purple-pale flex-shrink-0">
          ...
          {defLoading ? (
            <div className="flex items-center gap-2 text-gray-400 text-sm py-2">
              <svg className="animate-spin h-4 w-4 text-verint-purple" fill="none" viewBox="0 0 24 24">
        ...
        {/* Chat input */}
        <div className="px-4 py-3 border-t border-verint-purple-pale flex-shrink-0">
          <div className="flex gap-2 items-end">
            <textarea
              ...
              className="flex-1 resize-none rounded-lg border border-verint-purple-pale px-3 py-2
                         ...
                         focus:outline-none focus:ring-2 focus:ring-verint-purple/40 focus:border-verint-purple
